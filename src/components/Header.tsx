      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          <img
            src={displayLogo}
            alt={displayName}
            className="w-10 h-10 rounded-full object-cover border-2 border-[#E88D9F] shadow-xs shrink-0 bg-white"
            referrerPolicy="no-referrer"
          />
          <div>
            <h1 className="font-serif text-lg sm:text-xl font-bold tracking-tight text-[#3D3835] flex items-center gap-1.5">
              {displayName}
              <Sparkles className="w-4 h-4 text-[#E88D9F]" />
            </h1>
            <p className="text-[11px] text-[#D87085] tracking-wide font-medium">
              {displayTagline}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center flex-wrap gap-2 sm:gap-3">
          ...
          {showDemoSelector && (
            <div className="flex items-center gap-1.5 bg-[#FAF0ED] px-3 py-1.5 rounded-full border border-[#F2E3E1] text-xs">
              ...
              <select
                ...
                className="bg-white text-[#3D3835] text-xs py-0.5 px-2 rounded-full border border-[#F2E3E1] focus:outline-none focus:ring-1 focus:ring-[#E88D9F] font-medium"
              >
          ...
          {viewMode === 'staff' && currentStaff && (
            <div className="flex items-center gap-2 bg-[#FAF0ED] px-3 py-1.5 rounded-full border border-[#F2E3E1] text-xs">
              <span className="w-2 h-2 rounded-full bg-[#E88D9F] animate-pulse" />
              <span className="text-[#3D3835] font-semibold">{currentStaff.displayName}</span>
              <span
                className={`px-2 py-0.5 text-[9px] uppercase tracking-wider font-bold rounded-full ${
                  currentStaff.role === 'admin'
                    ? 'bg-[#E88D9F] text-white'
              ...
              <button
                ...
                className="text-[#9C948E] hover:text-[#D87085] ml-1 transition"
              >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 bg-white hover:bg-[#FAF0ED] text-[#3D3835] rounded-full text-xs font-bold uppercase tracking-wider border border-[#F2E3E1] transition shadow-2xs"
